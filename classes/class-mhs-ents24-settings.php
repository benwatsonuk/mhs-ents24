...
/**
 * MHS_Ents24_Settings Class
 *
 * @class MHS_Ents24_Settings
 * @version    1.0.0
 * @since 1.0.0
 * @package    MHS_Ents24
 * @author Jeffikus
 */
final class MHS_Ents24_Settings {
	/**
	...
	/**
	 * Main MHS_Ents24_Settings Instance
	 *
	 * Ensures only one instance of MHS_Ents24_Settings is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main MHS_Ents24_Settings instance
	 */
		...
		if ( is_array( $input ) && 0 < count( $input ) ) {
			...
			foreach ( $input as $k => $v ) {
				...
				if ( ! method_exists( $this, $method ) ) {
					if ( true === (bool) apply_filters( 'mhs-ents24-validate-field-' . $fields[ $k ]['type'] . '_use_default', true ) ) {
				...
				if ( '' != $method ) {
					add_filter( 'mhs-ents24-validate-field-' . $fields[ $k ]['type'], array( $this, $method ) );
				}

				$method_output = apply_filters( 'mhs-ents24-validate-field-' . $fields[ $k ]['type'], $v, $fields[ $k ] );
	...
	public function render_field( $args ) {
		...
		// Construct the key.
		$key           = MHS_Ents24()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
		...
		if ( isset( $args['type'] ) && ! in_array( $args['type'], (array) apply_filters( 'mhs-ents24-no-description-fields', array( 'checkbox' ) ) ) ) {
			if ( isset( $args['description'] ) ) {
				$description = '<p class="description">' . wp_kses_post( $args['description'] ) . '</p>' . "\n";
				if ( in_array( $args['type'], (array) apply_filters( 'mhs-ents24-new-line-description-fields', array(
					'textarea',
					'select'
				) ) ) ) {
	...
	public function get_settings_sections() {
		...
		$settings_sections['standard-fields'] = __( 'Basic Setup', 'mhs-ents24' );
//		$settings_sections['special-fields'] = __( 'Advanced Setup', 'mhs-ents24' );
		...
		return (array) apply_filters( 'mhs-ents24-settings-sections', $settings_sections );
	...
	public function get_settings_fields( $section ) {
		...
		switch ( $section ) {
			case 'standard-fields':
				...
				$settings_fields['textarea'] = array(
												...
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'mhs-ents24' )
											);
				$settings_fields['checkbox'] = array(
												'name' => __( 'Example Checkbox', 'mhs-ents24' ),
												'type' => 'checkbox',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'mhs-ents24' )
				...
				$settings_fields['radio'] = array(
												'name' => __( 'Example Radio Buttons', 'mhs-ents24' ),
												'type' => 'radio',
												'default' => '',
												'section' => 'standard-fields',
												'options' => array(
																	'one' => __( 'One', 'mhs-ents24' ),
																	'two' => __( 'Two', 'mhs-ents24' ),
																	'three' => __( 'Three', 'mhs-ents24' )
															),
												'description' => __( 'Place the field description text here.', 'mhs-ents24' )
											);
				$settings_fields['select'] = array(
													'name' => __( 'Example Select', 'mhs-ents24' ),
													'type' => 'select',
													'default' => '',
													'section' => 'standard-fields',
													'options' => array(
			...
			case 'special-fields':
				...
				$settings_fields['select_taxonomy'] = array(
					'name'        => __( 'Example Taxonomy Selector', 'mhs-ents24' ),
		...
		return (array) apply_filters( 'mhs-ents24-settings-fields', $settings_fields );
	...
	public function get_supported_fields() {
		return (array) apply_filters( 'mhs-ents24-supported-fields', array(
			'text',
			'checkbox',
			'radio',
			'textarea',
	...
	public function get_value( $key, $default, $section ) {
		$values = get_option( 'mhs-ents24-' . $section, array() );
	...
	public function get_settings( $section = '' ) {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				...
				$values = get_option( 'mhs-ents24-' . $v, array() );
